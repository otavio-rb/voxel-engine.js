...
export class Pig extends Animal {
    ...
    constructor(world: ProceduralWorld) {
        super(world, { width: 0.8, height: 0.8 }, 0xffaabb);
        this.buildModel(0xffaabb);
    }

    protected buildModel(color: number): void {
        this.legs = [];
        // Body
        this.createPart(0.8, 0.7, 1.2, color, 0, 0.4, 0);
        
        // Head
        const head = this.createPart(0.4, 0.4, 0.4, color, 0, 0.5, 0.5);
        // Snout
        this.createPart(0.25, 0.15, 0.1, 0xff88aa, 0, -0.05, 0.25, head);
        
        // Legs
        this.legs.push(this.createPart(0.2, 0.3, 0.2, color, -0.25, 0.15, 0.4));
        this.legs.push(this.createPart(0.2, 0.3, 0.2, color, 0.25, 0.15, 0.4));
        this.legs.push(this.createPart(0.2, 0.3, 0.2, color, -0.25, 0.15, -0.4));
        this.legs.push(this.createPart(0.2, 0.3, 0.2, color, 0.25, 0.15, -0.4));
    }
